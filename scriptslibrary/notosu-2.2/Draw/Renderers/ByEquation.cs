    ...
    public class ByEquation
    {
        ...
        public static string drawViaEquation(DrawInstance instance, double duration, EquationFunction equation, bool renderReceptor = true)
        {
            ...
            Parallel.ForEach(playfieldInstance.columns.Values, column =>
            {
                ...
                if (instance.ApplyHitLightingToNote)
                ...
                Parallel.ForEach(keysInRange, key =>
                     {
                         ...
                         do
                         {
                             ...
                             if (noteFade != null)
                             {
                                 var start = noteFade.value == 1 ? 0 : 1;
                                 initialFade = noteFade.InterpolateFadeByTime(currentTime + localIterationRate, start, noteFade.value);
                                 note.Fade(currentTime + localIterationRate, currentTime + localIterationRate, OsbEasing.None, initialFade);
                             }
                         ...
                         Parallel.ForEach(note.sliderPositions, part =>
                              {
                                  ...
                                  float fadeValue = 1;
                                  sprite.Fade(sliderCurrentTime - 10, 0);
                                  ...
                                  do
                                  {
                                      ...
                                      if (sliderFade != null)
                                      {
                                          var start = sliderFade.value == 1 ? 0 : 1;
                                          fadeValue = sliderFade.InterpolateFadeByTime(sliderCurrentTime + localIterationRate, start, sliderFade.value);
                                          if (sprite.OpacityAt(sliderCurrentTime) != fadeValue)
                                              sprite.Fade(sliderCurrentTime + localIterationRate, fadeValue);
                                      }
                                  ...
                                  if (instance.HoldRoationDeadzone != 9999)
